                    ...
                    <div class="card-body">
                        <div class="row mb-2">
                            <div class="col col-md-6 card-title">
                                <h4>Ingreso de stock a depósito</h4>
                            </div>
                        ...
                        <div class="col col-md-4 mr-2 mb-2 isBuy">
                            ...
                            <div class="col">
                                <button id="addStock" class="btn btn-primary float-right" onClick="confirmInput()">Enviar</button>
                            </div>
                        </div>
                    </div>
        ...
        result.items.forEach(function (result, index){
            ...
            $.ajax({
              ...
              success: function(data, textStatus, xhr) {
                var urlInput = "/api-stock/public/index.php/stock/register/";
                var depotInput = selectedDepotsDestino + '/';
                var typeArticleInput = result.split('/')[0] + '/';
                var idArticleInput = result.split('/')[1];
                var typeMoveInput = 'input';
                var dataInput = {
                    id: idArticleInput,
                    precio: priceArray[index],
                    cantidad: cantArray[index],
                ...
                $.ajax({
                    type: "POST",
                    url: urlInput + depotInput + typeArticleInput + typeMoveInput,
                    data: dataInput,
                    dataType: "json",
                    success: function (response) {
                        $('#success-alert').addClass('alert alert-success')
                            .add('span')
                            .text('Ingreso de stock a depósito creado');
                        $("#success-alert").fadeTo(2000, 500).slideUp(500, function(){
                            $("#success-alert").slideUp(500);
                            window.location.href ='../views/input_stock.php';
                        });
                    }
                });
              },
              error: function(xhr, textStatus, errorThrown) {
                    $('#danger-alert').addClass('alert alert-danger')
                        .add('span')
                        .text('Error al crear ingreso de stock a depósito');
        ...
        $.ajax({
            ...
            success: function (response) {
                console.log(response.result);
                    ...
                    <td> ${stock} </td>
                    <td> <input type="number" name="cant[]" max="${response.result[0].existencia}" min="1" value="" class="cant form-control cant" placeholder="Cant."> </td>
                ...
                $('.codes-liberate').click(function (e) { 
                    ...
                    window.localStorage.setItem("codeInput", JSON.stringify(codeInput));
