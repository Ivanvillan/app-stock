...
<div class="container">
    <div class="row">
        ...
        <div class="form-group mb-2 col-md-4">
            ...
            <select id="subtype-hardware" class="form-control" onChange="filterText()">
    ...
    <div class="row">
        <div class="col">
            <h3>Equipos</h3>
            <table id="table-hardware" class="table table-hover">
                <thead>
                    <tr>
...
<script>
    $(document).ready(function () {
        $.ajax({
            type: "GET",
            url: "/api-stock/public/index.php/products/get/hardware/0",
            dataType: "json",
            success: function (data) {
                ...
                for (let i=0; i < rows.length; i++){
                    html.push(
                 `<tr class="content" idHardware="${rows[i].id}">
                    <td class="nameHardware"> ${rows[i].nombre} </td> 
                    <td class="nameHardware"> ${rows[i].marca} </td> 
                    <td class="nameHardware"> ${rows[i].detalle} </td> 
                    <td class="nameHardware"> ${rows[i].descripcion} </td>
                    <td> <button class='info-hardware btn btn-info' data-toggle="modal" data-target="#modal-info"> <i class='fas fa-file-alt'> </i> </button> </td>
                    <td> <button class='edit-hardware btn btn-warning' data-toggle="modal" data-target="#modal-edit"> <i class='fas fa-edit'> </i> </button> </td>
                    <td> <button class='delete-hardware btn btn-danger' data-toggle="modal" data-target="#modal-delete"> <i class='fas fa-trash <'> </i> </button> </td>
                    </tr>`
                    );
                }    
                $('#table-hardware>tbody').html(html.join(''));
                $('.info-hardware').click(function (e) { 
                    $('.div-edit').remove();
                    $('.row-info').remove();
                    e.preventDefault();
                    var element = $(this)[0].parentElement.parentElement;
                    var idElement = $(element).attr('idHardware')
                    var url = "/api-stock/public/index.php/products/getbyid/hardware/"
                    $.ajax({
                        type: "GET",
                        url: url + idElement,
                        dataType: "json",
                ...
                $('.edit-hardware').click(function (e) { 
                    ...
                    $.ajax({
                        ...
                        dataType: "json",
                        success: function (response) {
                            var descriptionStr = response.result[0]["descripcion"];
                            var descriptionWord = descriptionStr.split(' | ')[1];
                            console.log(descriptionWord);
                            var PotMed = descriptionWord.split(' - ');
                            var edit = `
                            <div class="form-group div-edit div-category">
                            ...
                            <div class="form-group div-edit div-model">
                                ...
                                <input id="model-edit" type="text" class="form-control text-capitalize" value="${response.result[0].detalle}"></input>
                            </div>
                            <div class="form-group div-edit div-measure">
                                <label for="" class="col-form-label">Medida</label>
                                <input id="measure-edit" type="text" class="form-control text-capitalize" value="${PotMed[0]}"></input>
                            </div>
                            <div class="form-group div-edit div-potency">
                                <label for="" class="col-form-label">Potencia</label>
                                <input id="potency-edit" type="text" class="form-control text-capitalize" value="${PotMed[1]}"></input>
                            </div>
                            `;
                    ...
                    $('.confirm-update').click(function (e) { 
                        ...
                        data = {
                            ...
                            modelo: $("input[id=model-edit]").val(),
                            medida: $('input[id=measure-edit]').val(),
                            potencia: $('input[id=potency-edit]').val(),
                            codigo: $('input[id=code-edit]').val(),
                            categoria: $("select[id=subtype-edit]").val(),
                        };
                        console.log(data);
                        $.ajax({
                        type: "POST",
                ...
                $('.delete-hardware').click(function (e) { 
                    $('.div-edit').remove();
                    $('.row-info').remove();
                    e.preventDefault();
                    var element = $(this)[0].parentElement.parentElement;
                    ...
                    $('.confirm-delete').click(function (e) { 
                        e.preventDefault();
                        console.log(idElement);
                        var url = "/api-stock/public/index.php/products/drop/hardware/";
                        $.ajax({
                            type: "POST",
                            url: url + idElement,
                            dataType: 'json',
    ...
    $('.export-pdf').click(function(event) {
          ...
          doc.save("Equipos" + new Date().toISOString().replace(/[\-\:\.]/g, "") + ".pdf")
        });
    $('.export-excel').click(function(event) {
        ...
        //exclude: ".noExl",
        name: "Equipos",
        filename: "Equipos" + new Date().toISOString().replace(/[\-\:\.]/g, "") + ".xls",
        fileext: ".xls",
        exclude_img: true,
        exclude_links: true,
        exclude_inputs: true,
...
function filterText(){  
    var rex = new RegExp($('#subtype-hardware').val());
